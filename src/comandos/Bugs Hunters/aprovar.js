...
module.exports = {
  config: {
      name: "aprovar",
      aliases: ['aprovar'],
      description: "Dar uma falha reportada como aprovada.",
      example: "!aprovar",
      usage: '!aprovar ID <comentário>',
  ...
  run: async (bot, message, args) => {
	  ...
	  if(não_reproduz.includes('linha')) não_reproduz = não_reproduz.replace('linha', '\n')
	    ...
	    aprovado = `<a:Reproduz:812805434548158524> | **${usuario.user.username}** | ${args.slice(1).join(" ")}`
  ...
  if(não_reproduz === "false") {
	  ...
	  usuario.send(`Seu report de falha [**${descrição}**] foi finalizado como **aprovado!** ${re1} ${nore1} \n\n**Aprovado | ** ${aprovado}`)
      ...
      .setThumbnail('https://cdn.discordapp.com/emojis/840594888470036491.png?v=1')
      .setColor('#30d841')
      .setFooter('#' + args[0])
	  if(reproduz === "false"){
	  } else {
	  ...
	  embed.addField('Aprovado', aprovado)
  ...
  message.delete()
  message.reply('o report `' + args[0] + '` foi dado como **aprovado**').then(msg => {
                msg.delete({ timeout: 7000 });
            })
